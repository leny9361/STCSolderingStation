...
{
  FloatType input, error, dInput, output;
  ...
  if (now - pid->lastTime > pid->SampleTime)
  {
    ...
    if (pid->ITerm > pid->outMax * 100)
      pid->ITerm = pid->outMax * 100;
    else if (pid->ITerm < pid->outMin * 100)
      pid->ITerm = pid->outMin * 100;
    dInput = (input - pid->lastInput);

    /*Compute PID Output*/
    output = pid->kp * error + (pid->ITerm / 100) - pid->kd * dInput * 10;
...
void PID_SetTunings(PidType *pid, FloatType Kp, FloatType Ki, FloatType Kd)
{
  ...
  // SampleTimeInSec = ((FloatType) pid->SampleTime) / 1000;
  pid->kp = Kp;
